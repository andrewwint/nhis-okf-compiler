...
- **Safety scope.** Surfaces kept distinct by *deploy mode*. **Figures** are always aggregate-only: `nhis analyze`, the aggregate `nhis query` tools, and the **deployed** grounded agent explore **public, de-identified, aggregate** survey data — *not* medical or clinical advice, no individual-level inference, no health recommendations; every figure carries its survey-weighted basis and source, and a number is **never** read off raw rows. The deployed (retrieval-mode) agent registers no row tool and stays aggregate-only + pandas-free. **Raw rows** are a deliberately bounded, **local-only** capability (an approved scope change): the `nhis rows` CLI and the **local** agent's `inspect_rows` tool return a few columns of the **public, de-identified, top-coded** public-use microdata for inspection, each row **paired with its verified OKF column summary** (label, valid codes, universe) — never a fabricated meaning. Those rows are raw and **unweighted** — never population estimates — so a loud caveat precedes every call. The agent-supplied universe is **allow-list validated** (only `COLUMN <op> NUMBER` joined by `& | ( )` over known columns — the `injection-sink@universe-eval` seam) before it can reach `df.eval`. The invariant is enforced by tests: the deployed/retrieval agent stays row-free and pandas-free; the local row tool is capped + caveated.
